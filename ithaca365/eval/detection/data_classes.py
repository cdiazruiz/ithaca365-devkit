...
from ithaca365.eval.common.data_classes import MetricData, EvalBox
from ithaca365.eval.common.utils import center_distance
from ithaca365.eval.detection.constants import DETECTION_NAMES, ATTRIBUTE_NAMES, TP_METRICS
...
class DetectionMetrics:
    ...
    def __init__(self, cfg: DetectionConfig):
        ...
        self.cfg = cfg
        self._label_aps = defaultdict(lambda: defaultdict(lambda: defaultdict(float)))
        self._label_tp_errors = defaultdict(lambda: defaultdict(lambda: defaultdict(float)))
        self.eval_time = None

    def add_label_ap(self, detection_name: str, det_range: Tuple[float, float], dist_th: float, ap: float) -> None:
        self._label_aps[detection_name][str(det_range)][dist_th] = ap

    def get_label_ap(self, detection_name: str, det_range: Tuple[float, float], dist_th: float) -> float:
        return self._label_aps[detection_name][str(det_range)][dist_th]

    def add_label_tp(self, detection_name: str, det_range: Tuple[float, float], metric_name: str, tp: float):
        self._label_tp_errors[detection_name][str(det_range)][metric_name] = tp

    def get_label_tp(self, detection_name: str, det_range: Tuple[float, float], metric_name: str) -> float:
        return self._label_tp_errors[detection_name][str(det_range)][metric_name]

    def add_runtime(self, eval_time: float) -> None:
        self.eval_time = eval_time

    @property
    def mean_dist_aps(self) -> Dict[str, Dict[Tuple[int, int], float]]:
        ...
        return {class_name: {det_range: np.mean(list(m.values())) for det_range, m in d.items()} \
    ...
    def mean_ap(self) -> float:
        ...
        return float(np.mean([list(det_range_ap.values())[-1] for det_range_ap in \
            list(self.mean_dist_aps.values())]))
    ...
    def tp_errors(self) -> Dict[str, float]:
        ...
        for metric_name in TP_METRICS:
            ...
            for detection_name in self.cfg.class_names:
                last_range = tuple(self.cfg.class_range[detection_name][-1])
                class_errors.append(self.get_label_tp(detection_name, last_range, metric_name))
...
class DetectionRangeMetricDataList:
    """ This stores a set of MetricData in a dict indexed by (name, range, match-distance). """
    ...
    def get_class_data(self, detection_name: str) -> List[Tuple[DetectionMetricData, float]]:
        """ Get all the MetricData entries for a certain detection_name. """
        return [(md, r, dist_th) for (name, r, dist_th), md in self.md.items() if name == detection_name]

    def get_dist_data(self, dist_th: float) -> List[Tuple[DetectionMetricData, str]]:
        """ Get all the MetricData entries for a certain match_distance. """
        return [(md, r, detection_name) for (detection_name, r, dist), md in self.md.items() if dist == dist_th]

    def set(self, detection_name: str, det_range: Tuple[float, float], match_distance: float, data: DetectionMetricData):
        """ Sets the MetricData entry for a certain detection_name and match_distance. """
        self.md[(detection_name, det_range, match_distance)] = data

    def serialize(self) -> dict:
        return {key[0] + ':' + str(key[1]) + ':' + str(key[2]): value.serialize() for key, value in self.md.items()}

    @classmethod
    def deserialize(cls, content: dict):
        mdl = cls()
        for key, md in content.items():
            name, det_range, distance = key.split(':')
            mdl.set(name, eval(det_range), float(distance), DetectionMetricData.deserialize(md))
        return mdl